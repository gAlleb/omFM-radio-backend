...
let hls_listeners = {
  "total_listeners": { "omfm": 0, "cdp": 0, "rock": 0, "terra": 0, "core": 0, "coma": 0, "chill": 0 },
  "omfm": [], "cdp": [], "rock": [], "terra": [], "core": [], "coma": [], "chill": []
};
...
async function fetchListenerData() {
  try {
    ...
    const rockData = await fetchData(streamUrls.rock);
    const terraData = await fetchData(streamUrls.terra);
    const coreData = await fetchData(streamUrls.core);
    const chillData = await fetchData(streamUrls.chill);
    const comaData = await fetchData(streamUrls.coma);

    // Ensure hls_listeners is initialized
    if (!hls_listeners) {
      hls_listeners = {
        "total_listeners": { "omfm": 0, "cdp": 0, "rock": 0, "terra": 0, "core": 0, "coma": 0, "chill": 0 },
        "omfm": [], "cdp": [], "rock": [], "terra": [], "core": [], "coma": [], "chill": []
      };
    } else if (!hls_listeners.total_listeners) {
      hls_listeners.total_listeners = { "omfm": 0, "cdp": 0, "rock": 0, "terra": 0, "core": 0, "coma": 0, "chill": 0 };
    }


    hls_listeners.total_listeners.rock = rockData.length;
    hls_listeners.total_listeners.terra = terraData.length;
    hls_listeners.total_listeners.core = coreData.length;
    hls_listeners.total_listeners.chill = chillData.length;
    hls_listeners.total_listeners.coma = comaData.length;

    hls_listeners.rock = rockData;
    hls_listeners.terra = terraData;
    hls_listeners.core = coreData;
    hls_listeners.chill = chillData;
    hls_listeners.coma = comaData;
...
let total_np = [
    {
      ...
      data: { np: np_data, hls_listeners: hls_listeners, spotifyToken: spotifyToken },
    },
    {
      channel: "station:cdp",
      data: { np: np_data_cdp, hls_listeners: hls_listeners, spotifyToken: spotifyToken },
    },
  ];
res.json(total_np);

});
app.get('/hls_listeners', (req, res) => {
  // try {
...
/////////////////////////////////////////////////////////////
app.post('/hls_stat', (req, res) => {

  const newData = req.body;

  if (newData) {
    if (!hls_listeners) {
      // If hls_listeners is completely empty, initialize it
      hls_listeners = {
        "total_listeners": { "omfm": 0, "cdp": 0, "rock": 0, "terra": 0, "core": 0, "coma": 0, "chill": 0 },
        "omfm": [], "cdp": [], "rock": [], "terra": [], "core": [], "coma": [], "chill": []
      };
    ...
    // Merge total_listeners
    if (newData.total_listeners) {
      hls_listeners.total_listeners.omfm = newData.total_listeners.omfm || 0;
      hls_listeners.total_listeners.cdp = newData.total_listeners.cdp || 0;
    }

    // Merge stream data
    hls_listeners.omfm = newData.omfm || [];
    hls_listeners.cdp = newData.cdp || [];

    // console.log('hls_stat updated hls_listeners with:', newData);
  } else {
    console.error('Invalid hls_stat data received:', newData);
  }
